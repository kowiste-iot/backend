...
func (h *TenantHandler) TenantIDMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		tenantID, exist := c.Params.Get("tenantid")
		if !exist || tenantID == "" {
			httputil.NewErrorResponse(c, errors.NewBadRequest("Tenant ID is required", nil))
			c.Abort()
			return
		}
		...
		branchID, exist := c.Params.Get("branchid")
		if !exist || branchID == "" {
